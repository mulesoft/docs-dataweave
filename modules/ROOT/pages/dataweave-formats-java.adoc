...
* Default constructor (required if your class needs to be instantiated)
* Getters for all properties (required if your class needs to be read)
* Setters for all properties (required if your class needs to be instantiated)

[[java17_examples]]
== Examples of Java Support

The following examples run using Java 17 and using xref:dataweave-versioning-behavior.adoc[language level 2.6].
...
To solve conflicts, ensure you: 

* Update your class definitions to follow the POJOs structure. 
+
We encourage you to adopt this option.
...
+
This approach is a workaround and works for customer domain classes only. Note that the usage of this compatibility flag may not affect future versions.