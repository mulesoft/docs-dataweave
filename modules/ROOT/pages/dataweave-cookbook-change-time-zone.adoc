...
:page-aliases: 4.3@mule-runtime::dataweave-cookbook-change-value-of-a-field.adoc

The following example uses the `>>` operator and a <<timezone-ids, time zone ID>> to change the time zone. It also uses the formatting characters `uuuu-MM-dd\'T\'HH:mm:ss.SSS` to modify the format. The `uuuu` characters represent the year.

.DataWeave Script:
[source,dataweave,linenums]
----
include::partial$cookbook-dw/change-time-zone/transform.dwl[]
----

.Output XML:
[source,xml,linenums]
----
include::partial$cookbook-dw/change-time-zone/out.json[]
----
