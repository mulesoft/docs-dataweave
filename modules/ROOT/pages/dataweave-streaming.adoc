...
NOTE::
Exceptions are not handled when you set `deferred = true`. For example, you can see this behavior in Studio when a flow throws an exception. If you are running an application in Studio debug mode and an exception occurs in a Transform Message component when `deferred = true`, the console logs the error, but the flow does not stop at the Transform Message component.
