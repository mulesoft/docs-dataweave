...
If you want to set different reader properties to the inputs you are using you need to create a properties file with the suffix `{fileName}-config.properties` in the same directory of your input. So let us say your input is called `payload.csv` then you could create a `payload-config.properties` file which will contain all the reader properties needed for running your scenario.
