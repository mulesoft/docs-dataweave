...
* Auto-completion filters out directives that aren't visible from the current file. For example, `private` helpers defined in another file don't appear in the suggestion list.
...
For the full visibility model, including definitions for `private`, `internal`, and components, and how the DataWeave Maven plugin generates the component descriptor, see xref:dataweave::dataweave-scope-visibility.adoc[].
