...
=== Using reader properties for your inputs

If you want to set different reader properties to the inputs you are using you need to create a properties file with the suffix `{fileName}-config.propertes` in the same directory of your input. So let us say your input is called `payload.csv` then you could create a `payload-config.properties` file which will contain all the reader properties needed for running your scenario.
This also applies while testing your DataWeave Mappings.
...
If something is not working as expected or an error is thrown and you want to get more details or raise an issue and get help from the dev team, you should consider attaching the log output of the DataWeave Language Server. This will help us (and you) to get better understanding of what went wrong.

To do so please follow these steps:

. Go to your View menu.
. Select Output, this will open your Output View below your editor.
. On the Output view, click the selection combo and choose Mule DX DataWeave LS.
. Copy the contents of your log output.
