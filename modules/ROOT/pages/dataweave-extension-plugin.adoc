...
If you want to set different reader properties to the inputs you are using you need to create a properties file with the suffix `{fileName}-config.propertes`. So let us say your input is called `in0.csv` then you could create a `in0-config.properties` file which will contain all the reader properties needed for running your scenario.
