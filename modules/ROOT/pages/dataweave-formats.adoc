...
:page-aliases: 3.8@mule-runtime::dataweave-formats.adoc
...
|`int/Short/Long/BigInteger/Flat/Double/BigDecimal`|xref:dataweave-types.adoc#number[:number]
...
|`writeDeclaration`| boolean | true | Defines if the XML declaration will be included in the first line (available since Mule 3.8.4)
|===
...
|`missingValues`| string | spaces a| (since 3.8.1) How missing values are represented in the input data:
