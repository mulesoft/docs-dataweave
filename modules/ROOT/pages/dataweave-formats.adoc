...
:page-aliases: 3.9@mule-runtime::dataweave-formats.adoc
...
image::dataweave-formats-580be.png["A reader configuration table for CSV files showing settings for headers, separators, and escapes."]
...
+
image::dataweave-formats-4a556.png["A metadata type selection screen showing options for CSV and XML data types."]
...
image::dataweave-formats-exceltable.png["A table in an Excel format displaying IDs and names starting from row 1."]
...
image::dataweave-formats-excell-reader.png["A reader configuration dialog for Excel with options for handling empty lines and setting table offsets."]
...
+
image::dataweave-formats-excel-metadata.png["A user interface for selecting metadata types with options for Excel formats and a table listing fields like name, address, and age."]
...
image::dataweave-formats-6e5e4.png["A reader configuration dialog with options for handling null values and optimizing for speed."]
...
|`skipNullOn`| string | | Possible values = `elements`/`attributes`/`everywhere`. See <<skip-null-on-for-xml>>
...
|`bufferSize`| number | 153600 | The size of the buffer writer
|`inlineCloseOn` | string | | When the writer should use inline close tag. Possible values = empty/none
|`skipNullOn`| string | | Possible values = `elements`/`attributes`/`everywhere`. See <<skip-null-on-for-json>>
...
* By Providing a sample file
* By pointing to a <<Flat File, flat-file>> schema file
* Via a graphical editor that allows you to set up each field manually

+
image::dataweave-formats-27b3c.png["A metadata selection screen with various data type configurations including JSON, XML, and flat files."]
