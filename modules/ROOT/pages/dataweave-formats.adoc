...
DataWeave can read input data as a whole by loading it into memory or by indexing it in local storage and, for some data formats, read data sequentially in parts by streaming the input. The indexed reading strategy implements a mechanism to avoid out-of-memory issues when processing large files. The streaming strategy can improve performance but impacts access to file.

[[streaming]]
[%header%autowidth.spread,cols="a,a,a", options="header"]
|===
| Read Strategy | Behavior | Supported Formats
<.^| In-Memory
| Enables random access to data by parsing the entire document. When using this strategy, a DataWeave script can access any part of the resulting value at any time.
| DataWeave can read all supported formats using this strategy.
<.^| Indexed
| Enables random access to data by parsing the entire document.

When using this strategy, a DataWeave script can access any part of the resulting value at any time. When processing a `String` with a size larger than 1.5 MB, DataWeave automatically splits the value in chunks to avoid out-of-memory issues.

You can configure the maximum size DataWeave uses to split strings by modifying the following system property:

```
com.mulesoft.dw.max_memory_allocation
```
//can we show an example of a valid value? Is 4096 (chars) the default?

Note that splitting strings and accessing them through local storage introduces performance penalizations. If your environment has enough resources to load all content into memory, you can also disable the string management feature completely by setting the value of the following system property to `false`:
