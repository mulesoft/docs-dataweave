...
DataWeave uses AsciiDoc text format to document your functions, modules, types, and annotations. 

=== Documenting Your DataWeave Functions

DataWeave defines a template for documenting your DataWeave function code that has three main sections: 

* A description.  
* Parameters table for any function signatures that include parameters.
* Example code that includes sections for any input, the DataWeave script, and the resulting output. 

To separate the sections, the template uses AsciiDoc heading grammar: 
...
The template provides guidance on documenting the function. As noted, some sections are optional. 

.DataWeave Function Template
[source,DataWeave,linenums]
----
/**
...
This section provides a description of the function. The section starts at the top of the comments and extends to the start of a section, such as  `=== Parameters`.

A description consists of two parts: 

* Short description: The first paragraph, which provides the main description. 
* Long description: Any additional information about the function that begins two line breaks below the short description.

Each part is useful for the auto-generated documentation.

==== Parameters Section

This optional section describes parameters of the function.

The template uses Asciidoc table format to document each parameter in a table row.

==== Example Section

You can use zero or more `Example` sections to provide all examples needed to illustrate how your function works.

The section contains the following optional subsections: 

* Input
* Source
* Output

All subsections use the same template, an optional description, and an optional code section.
...
The following example shows the xref:dw-core-functions-log.adoc[log] function documentation.
...
DataWeave module documentation does not follow a structured template.

The only requirement is to place the documentation above the DataWeave version tag.

The following example documents the xref:dw-core.adoc[dw::Core] module:
...
Annotation and Types documentation does not follow a structured template.

The only requirement is to write it above the Annotation or Type definition.
...
The following example documents the `EncodingSettings` type found in the xref:dw-dataformat.adoc[dw::extension::DataFormat] module.
