...
| `parts` | `Number` values for `hour`, `minutes`, and `seconds` fields, and a
            ...
            through 23 for the `hour`, 0 through 59 for `minutes`, and 0 through 59 (not inclusive; it can accept 59.99, for example) for `seconds` fields. The `timezone` must be a valid `TimeZone` value,
