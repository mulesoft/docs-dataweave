...
The following example shows how to transform multipart input into multipart outputs but with a different format. The DataWeave script uses both `^mimeType` and `^mediaType` selectors to generate multipart content inside other multipart content.
