...
The `^mimeType` selector returns the MIME type of a value without parameters, for example, `application/json`. +
The `^mediaType` selector by contrast includes any other related parameters related to the MIME type, for example, `application/json;charset=UTF-16`.

The following example shows how to transform multipart inputs into multipart outputs with a different format. The DataWeave script uses both the `^mimeType` and `^mediaType` selectors to generate multipart content inside other multipart content.

=== Input
The multipart input has Content-Type `application/json`:
...
The DataWeave script transforms the multipart input into multipart output by defining `output multipart/form-data` MIME type. The multipart output data contains three parts:
* `orderAck` +
Contains a text with an acknowledge of the receipt of an order. The Content-Type is `text/plain`.
* `order` +
Contains the order itself. The Content-Type is `payload.parts.order.content.^mimeType`. The ^mimeType` selector becomes useful to generate the Content-Type field.
* `originalContent`
Contains the original content which is multipart. The Content-Type is `payload.^mediaType`. The ^mediaType selector includes useful metadata such as the original boundary.
