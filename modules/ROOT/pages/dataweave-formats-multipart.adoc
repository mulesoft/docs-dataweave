...
[[example2]]
=== Example: Generate Multipart Form Data Text with a Random Boundary
This example shows how the dw::Multipart::generateBoundary() function can be used to generate a random boundary string value for use in contructing valid multipart text that can be read as data that is valid multipart/form-data MIME type data

==== Input

In this example, the input data is similar to the input in Example 1, but here the boundary string is replaced with the result of the dw::Multipart::generateBoundary function. 

==== Source
This script parses the data variable, which is structured as n object of parts. In this example, think of each part as being the result of some previous connecor operation. The first part is simple text data, while the other parts represent application/json and application/xml type input that has an object structure. 

The `separator` variable uses `dw::Multipart::generateBoundary(7)` to generate a random string of 7 characters. In the protocol, the multipart/form-data boundary can be any string of any length, but the value must not occur anywhere else in the message text. 

The multipartString variable is constructed from the data variable to write out the required text to define each part of valid multipart/form-data MIME type text. It uses the `reduce` function to combine each part together with the correct boundary string and multipart/form-data part. The mutlipartString implementation uses the dw::Multipart::generateBoundary function to generate a random boundary. This function accepts an int value to decide how many characters to make the boundary string. Once generated, the same boundary string must be used to differentiate the boundary between each part of the multipart text. Each boundary is prefixed by two hyphens `--`. The last boundary (which denotes the end of the message) is prefixed and suffixed by two hyphens `--`.
...
var separator = Multipart::generateBoundary(5)

var data = {
	part: {"form-type": "text", name: "text", "content-type":"text/plain", value: "Book"},
	part: {"form-type": "file", name: "file1", "file-name": "a.json", "content-type":"application/json", value:
		write({
			"title": "Java 8 in Action",
			"author": "Mario Fusco",
			"year": 2014
	...
	part: {"form-type": "file", name: "file2", "file-name": "a.html", "content-type":"text/html", value:
			"<title> Available for download! </title>"
	},		
}

var multipartString = data.*part reduce (
    (part, acc="") -> 
'--$(separator) \nContent-Disposition: form-data; name=$(part.name)
Content-Type: $(part."content-type")
$(part.value)\n' ++ acc
)
//Add last ending boundary
++ '\n\n--$(separator)--'

---
//Read a multipart string into valid multipart/form-data output. 
read(multipartString, "multipart/form-data", {boundary: separator})
