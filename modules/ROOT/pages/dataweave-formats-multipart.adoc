...
|`defaultContentType`  |`String`|`'application/octet-stream'`|Sets the default Content-Type to use on parts of the `multipart/*` format. When set, this property takes precedence over the setting for the https://docs.mulesoft.com/dataweave/latest/dataweave-system-properties[system property] `com.mulesoft.dw.multipart.defaultContentType`.
