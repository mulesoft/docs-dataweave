...
The `^mimeType` selector returns the MIME type of a value without properties, for example, `application/json`. The `^mediaType` selector, by contrast, provides the MIME type and any other properties related to it, for example, `application/json;charset=UTF-16`.
