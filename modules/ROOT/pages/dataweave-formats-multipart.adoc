...
* `orderAck`: Contains a text with an acknowledgement of the receipt of an order. The Content-Type is `text/plain`.

* `order`: Contains the original order itself. The Content-Type is `payload.parts.order.content.^mimeType`. The ^mimeType` selector becomes useful to generate the Content-Type field.

* `partner`: Contains the original partner itself. Notice this time, the Content-Type is `payload.parts..^mediaType`. The ^mediaType selector includes also the separator which is needed to parse the `application/csv` content.
