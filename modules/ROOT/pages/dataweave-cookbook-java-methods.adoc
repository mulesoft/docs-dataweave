...
The following example shows an API that returns a greeting message based on the input `uriParam`.
The Java bridge calls a static method within the `my.class.org.Greeter` class that builds the response message.
...
. Run the following curl command: `+http://localhost:8081/greet/Username+`.
