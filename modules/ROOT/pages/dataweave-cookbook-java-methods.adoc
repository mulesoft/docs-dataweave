...
:page-aliases: 4.1@mule-runtime::dataweave-cookbook-java-methods.adoc
...
The DataWeave example below first imports the `MyClass` class, then creates a new instance of the class and calls its instance variable `foo`. Note you cannot call the private method `getFoo()` with DataWeave.
