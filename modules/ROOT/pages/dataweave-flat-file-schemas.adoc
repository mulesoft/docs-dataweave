...
:page-aliases: 3.8@mule-runtime::dataweave-flat-file-schemas.adoc

include::partial$eol-warning.adoc[]

xref:index.adoc[DataWeave] allows you to process several different xref:dataweave-formats.adoc[types of data]. For most of these types, you can import a schema that describes the input structure in order to have access to valuable metadata at design time. See xref:6@studio::input-output-structure-transformation-studio-task.adoc[To Define Input and Output Structure of a Transformation].
...
If you have multiple structures or segment definitions in an FFD, when you apply your schema to an metadata description on a xref:index.adoc[Transform Message component] you'll need to specify which one you want to use.
...
|tagLength |Number of columns in segment identifier tags. Required for flat file structures. Must be greater than zero.
|data |List of segments (and groups) in the structure
|===

[NOTE]
The *tagStart* and *tagLength* parameters are required when using flat file structures. Using both is the only way for the parser to distinguish different segments. The only supported value for tagStart now is 0.
...
. Drag an xref:3.8@mule-runtime::http-connector.adoc[HTTP Connector] from the palette to your empty canvas, then a *Transform Message Component*.
+
