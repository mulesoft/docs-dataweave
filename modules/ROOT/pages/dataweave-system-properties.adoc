...
Available for language levels: 2.6, 2.7, 2.8, 2.9
...
* Default:
  ...
  ** `true` for language levels: 2.9

Available for language levels: 2.8, 2.9


//com.mulesoft.dw.java.output_types_as_string//
|`com.mulesoft.dw.java.output_types_as_string`|

When set to `true`, DataWeave scripts with a Java output MIME type that return a type return the string representation of the type. If set to `false`, the scripts return a Java class constructed from the corresponding DataWeave type class.

* Type: `Boolean`

* Default:
  ** `true` for language levels: 2.0, 2.1, 2.2, 2.3, 2.4, 2.5, 2.6, 2.7, 2.8, 2.9

Available for language levels: 2.0, 2.1, 2.2, 2.3, 2.4, 2.5, 2.6, 2.7, 2.8, 2.9
...
|`com.mulesoft.dw.stableOrderBy`|
When set to `true`, DataWeave throws an `InvalidComparisonException` if the `orderBy` function attempts to compare values of different types. If set to `false`, coercions are used when comparing values of different types, which can cause that the resulting order isn't the expected one as in:
.DataWeave script:
[source,xml,linenums]
----
%dw 2.4
...
Available for language levels: 2.9
...
|`com.mulesoft.dw.xml_reader.allowIndexedLatin1`|
When set to `true`, DataWeave lets you use the indexed reader for Latin1(ISO-8859-1) encoding. Use this only for backward compatibility, as Latin1 isn't fully supported.
...
Available for language levels: 2.4, 2.5, 2.6, 2.7, 2.8, 2.9
