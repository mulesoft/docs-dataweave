...
|`com.mulesoft.dw.java.enabled_class_definition_lookup`|
"When set to `true` the JavaModuleLoader honour the public interface definition of the given bean instance to access properties via reflection.
The default value evaluates the current JDK version used and the language level. The value is `true` when the used JDK is 17+ and the language version is bigger than 2.10.
If it is set to `false`, the JavaModuleLoader uses the specific bean's class definition to access properties via reflection which could ends on accessibility issues.
...
which can cause that the resulting order isn't the expected one as in:
