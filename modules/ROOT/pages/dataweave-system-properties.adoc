...
|`com.mulesoft.dw.stableOrderBy`|
When set to `true`, DataWeave throws an `InvalidComparisonException` if the `orderBy` function attempts to compare values of different types. If set to `false`, coercions are used when comparing values of different types, 
