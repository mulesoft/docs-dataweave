...
When set to `true`, `dw::core::Binaries::fromBase64` and `dw::core::Binaries::toBase64` functions do not load the entire function argument (`String` or `Binary`) into memory.
...
Available for language levels: 2.11
...
When this property is set to `false`, DataWeave indexed readers for CSV do not keep string value references in memory. This feature may reduce performance, but it helps prevent out-of-memory errors.
...
When this property is set to `true`, DataWeave escapes quotes in CSV.
...
When set to `true` the JavaModuleLoader honour the bean definition by using the setter/getter methods to access properties via reflection at object value selectors.If it is set to `false`, the JavaModuleLoader uses the setter/getter methods to access properties if exists or it uses the field accessor as default. 
...
When set to `true` dw scripts with java output mimetype that return a type will return the string representation of the type. If set to false it will return a Java class constructed from the corresponding DW type class
...
When set to `true`, returns `1` when calling `sizeOf()` function for any `Number` value.
...
When set to `true`, DataWeave throws an `InvalidComparisonException` if the orderBy function attempts to compare values of different types.
...
the code point is not a valid unicode character.
...
When this property is set to `true`, DataWeave allows using the indexed reader for Latin1(ISO-8859-1) encoding. This should only be used for backwards compatibility as Latin1 is not fully supported
