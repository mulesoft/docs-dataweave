...
Valid values are `true` or `false`.
|`recordTerminator`  |`String`|`null`|Line break for a record separator. Valid values: `lf`, `cr`, `crlf`, `none`. This property is used as a separator only when there are multiple records. Values translate directly to character codes, and `none` leaves no termination on each record.
