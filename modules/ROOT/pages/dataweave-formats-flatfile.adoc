...
|`substituteCharacterAsMissingValue`  |`Boolean`|`false`|If set to `true` and the `missingValues` property is set to `none`, with `useMissCharAsDefaultForFill` set to `true`, the flat file parser handles the `0x1A` (SUB) character as the missing value. +
