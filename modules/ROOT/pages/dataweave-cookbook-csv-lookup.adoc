...
The value of the variable `country_codes` is a CSV file that maps calling codes to country codes.
...
The following DataWeave script reads each phone number in the payload and uses the `filter` function to search for the calling code within the variable `country_code` that matches the calling code in the phone number. Then the `map` function gets the country code that corresponds to the calling code and returns the results to an array of JSON objects.
