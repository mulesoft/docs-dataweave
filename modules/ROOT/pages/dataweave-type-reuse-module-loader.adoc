...
As previously mentioned, this is a translation between the XML schema type system to our own. Therefore, there are some general rules and limitations that apply that are important when working with these types.
The translations between the XML schema model to DataWeave are described below using different XSD files and showing the translation result in DataWeave code. Notice that these files are generated dynamically and are not visible, though they are useful to learn how to consume types from generated XSD files.
...
XSD namespaces are carried to types and elements definition to make sure that the structure matches exactly. 
The following example defines a specific `targetNamespace`:
