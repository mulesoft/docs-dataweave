...
You can declare and use types within the same DataWeave script (see xref:dataweave-type-system.adoc[]), or you can reuse types from different sources by loading them with a specific module loader. DataWeave currently supports loading types from:
