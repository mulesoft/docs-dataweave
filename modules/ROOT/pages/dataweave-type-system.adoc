...
You can declare and use types within the same DataWeave script (see xref:dataweave-type-system.adoc[]), or you can reuse types from different sources by loading them with a type-specific module loader. DataWeave currently supports loading types from:

* xref:dataweave-type-reuse-modules.adoc[]
* xref:dataweave-type-reuse-json-schema.adoc[]
* xref:dataweave-type-reuse-xml-schema.adoc[]
* xref:dataweave-type-reuse-java-classes.adoc[]

Module loaders are used to load files as DataWeave modules, where different parts of those files can be loaded as DataWeave functions, variables, types, and namespace definitions. Using a prefix such as `java!` or `xmlschema!` in the import directive tells DataWeave which module loader to use when loading the Java class or the XML schema as a DataWeave module.