...
`DocType` is a custom DataWeave data type for XML that is used to identify a doctype declaration.
...
To read a doctype declarations, set the xref:dataweave-system-properties.adoc[system property] `com.mulesoft.dw.xml_reader.parseDtd`.

During the reading phase, DataWeave parses the doctype declarations and stores the content as `metadata` of the root element.

You can use the xref:dataweave-selectors.adoc[metadata selector] to access it.

NOTE::
DTDs are disabled by default even if their declarations are read or written. For details, see the xref:dataweave-formats-xml.adoc#properties[XML reader property] `supportDtd`.
...
Refer to https://docs.mulesoft.com/dataweave/latest/dataweave-formats[DataWeave Formats] for more detail on available reader and writer properties for various data formats.
...
The following DataWeave script transforms the XML input payload with a `DOCType` value into JSON.
...
[[example14]]
=== Example: Transform `DocType` Value to a String Representation

This example uses https://docs.mulesoft.com/dataweave/latest/dw-dtd[Dtd Module (dw::xml::Dtd)] to transform
