...
:page-aliases: 4.2@mule-runtime::dw-core-functions-maxby.adoc
...
The items need to be of the same type. `maxBy` throws an error if they are not,
and it returns `null` when the array is empty.
