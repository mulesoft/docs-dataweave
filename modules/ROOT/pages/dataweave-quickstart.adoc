...
:page-aliases: 3.9@mule-runtime::dataweave-quickstart.adoc
...
. Drag an xref:3.9@mule-runtime::http-connector.adoc[HTTP Connector] from the palette onto your empty canvas, followed by a *Transform Message* component, then another HTTP Connector.
+
image::dataweave-quickstart-5a4cb.png["A schematic view of a data transformation process."]
...
+
image::dataweave-quickstart-a56f0.png["An error message indicating a missing 'config-ref' attribute in an HTTP configuration screen."]
...
+
image::dataweave-quickstart-702fd.png["A metadata configuration screen in a data transformation tool with an option to add metadata."]
. Click the *Edit* icon that appears next to the dropdown.
+
image::dataweave-quickstart-8ea02.png["An error message on an HTTP configuration screen indicating a missing attribute."]
+
. Click the green plus sign to *Add* a new metadata type.
+
image::dataweave-quickstart-c8ae5.png["A user interface for selecting metadata types with options to add or delete types."]
+
. Name the metadata type `XML-order-input` and click *Create type*.
+
image::dataweave-quickstart-22a00.png["A user interface for creating a new XML metadata type."]
+
...
+
image::dataweave-quickstart-26f37.png["A metadata type selection interface for XML data related to orders, showing fields like buyer and address."]
+
[TIP]
====
...
image::dataweave-quickstart-4a1db.png["A data transformation interface showing input fields for product details and output in JSON format."]
====
...
+
image::dataweave-quickstart-6d4b7.png["A configuration screen for an HTTP request with settings for host, port, and base path."]
...
+
image::dataweave-quickstart-3c9c7.png["A configuration screen for an HTTP request with fields for URL settings and method POST."]
...
image::dataweave-quickstart-f39a5.png["A data transformation interface showing XML input and JSON output configurations."]
====
...
+
image::dataweave-quickstart-flows-from-raml.png["A project directory structure for a DataWeave application with options for managing metadata."]
+
...
+
image::dataweave-quickstart-set-payload.png["A configuration screen for setting payload in a message flow with MIME type settings."]
...
image::dataweave-quickstart-1984d.png["A Transform Message component showing a mapping between the input and output."]
...
+
image::dataweave-quickstart-c1e53.png["A data transformation interface detailing fields for input and output information in JSON format."]
+
...
+
image::dataweave-quickstart-4abf1.png["A data transformation interface showing fields for input and output information in JSON format."]
...
See how the expression that evaluates `size` has changed--it uses the xref:dataweave-language-introduction.adoc#unless-otherwise[Unless and Otherwise operators] to set the value to `M` _unless_ the buyer's name is Homer Simpson, in which case the shirt size, expressed as `size` becomes `XXL`.
...
+
image::dataweave-quickstart-07f8a.png["Preview button icon", 5%, 5%]
+
...
+
image::dataweave-quickstart-59e9c.png["A JSON output preview mapping output's details like address and email from an order."]
+
...
+
image::dataweave-quickstart-75ca5.png["A snippet of an XML input sample."]
+
...
+
image::dataweave-quickstart-e4466.png["A flow of a data transformation process involving HTTP and error handling."]
...
+
image::dataweave-quickstart-2196b.png["A user interface for transforming message data with a focus on output payload structure."]
...
+
image::dataweave-quickstart-be7fd.png["A preview screen prompting the creation of sample data for execution"]
...
image::dataweave-quickstart-7c173.png["An output payload preview in a data transformation tool."]
...
image::dataweave-quickstart-fb3b7.png["A Java output payload displaying a list of attributes"]
...
image::dataweave-quickstart-c8b4f.png["A CSV output preview."]
...
image::dataweave-quickstart-6381c.png["A JSON output preview"]
