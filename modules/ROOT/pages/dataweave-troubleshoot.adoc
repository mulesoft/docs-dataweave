...
For example, posting a multipart payload with filename=`不明.txt` without setting the `allowutf8` property to `true` would produce the following garbled text for the `filename` field:
