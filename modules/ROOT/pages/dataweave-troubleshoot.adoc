...
+
The default directory is what is set in the `java.io.tmpdir` property.
...
In order to handle large payloads, DataWeave generates some data which is handled in memory unless it exceeds a certain configurable limit.
When that happens the data is stored on disk as output, input, and buffer files in a temporary directory. See xref:dataweave-memory-management.adoc[DataWeave Memory Management] for further details.

Once the streams referencing those files are closed the files are released. The closure normally happens when flows complete their execution, so long-running and particularly concurrent executions could involve having many of these files living at once.

To avoid the exception `No space left on device` more resources are needed to be able to run the app, or the app needs to be reviewed to reduce its resource consumption.

Sometimes bugs can lead to them not being removed even after the flow has completed.
If this case isn't fixed in the latest update, there’s no way to work around the issue, and this can be reported to the Mulesoft support team.

=== Closed Streams
The input data that makes its way to DataWeave is usually a stream handled by the Mule Runtime: they will add auto-closing and repeatable capabilities by generating “cursors” over them.
Sometimes these streams are closed prematurely by the time they reach a DataWeave script, and it can be hard to understand who has closed it.
In these circumstances, the script will fail with an error similar to: "Cannot open a new cursor on a closed stream".

If this case isn't fixed in the latest update, there’s no way to work around the issue, but there is a way to determine which component is closing the stream prematurely, so the issue can be properly reported to the Mulesoft support team.
Using the `com.mulesoft.dw.track.cursor.close` system property, the error will start showing the stack trace from the moment the stream was closed and therefore point to whoever triggered the closing.
