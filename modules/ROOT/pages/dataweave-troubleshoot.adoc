...
Unlike Mule runtime, a standalone DataWeave runtime, such as the one in the https://dataweave.mulesoft.com/learn/[DataWeave Playground+], can process a valid MIME type set through the `input` directive in the same DataWeave script.
