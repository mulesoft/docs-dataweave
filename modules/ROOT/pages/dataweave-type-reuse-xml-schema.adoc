...
You can import XML schema files (`.xsd`) inside your DataWeave script as modules by using the `xmlschema!` module loader. This loader enables you to use types that are declared on your schema inside DataWeave directly.
...
The following example shows how to look for an XSD file in the path `org/weave/myfolder/User.xsd` inside your DataWeave script:
...
To import the XML schema types, as shown above, use the following syntax, where:
...
As previously mentioned, the XML schema type system translates to DataWeave type directives. Therefore, as the two type models are considered different, the resulting DataWeave Type might not represent exactly the same XML Model.
...
Each root element (`<xs:element>`) declared inside your XSD file generates the following:

* A DataWeave `type` directive with the element name followed by the `ElementDeclaration` extension.
...
The use of imports or includes inside an XSD schema file results in a DataWeave module which contains an `import` directive that uses the XML schema module loader to point to the schema file located at the specified `schemaLocation`. This location must be declared relative to the main schema. Notice how the following examples use xref:dataweave-type-selector.adoc[]:
...
The following example shows how to set the specific `targetNamespace` where your elements exist. This namespace, which translates to DataWeave, is required if you want to select and navigate said types. 
