...
| xref:dw-math-functions-decimaladd.adoc[decimalAdd] | Performs number addition with rounding specified by the operation context.
| xref:dw-math-functions-decimaldivide.adoc[decimalDivide] | Performs number division with rounding specified by the operation context. If precision is set to `0` (unlimited precision)
and the result has an infinite decimal expansion, the function returns an error.
| xref:dw-math-functions-decimalmultiply.adoc[decimalMultiply] | Performs number multiplication with rounding specified by the operation context.
| xref:dw-math-functions-decimalpow.adoc[decimalPow] | Returns a number with value base^exponent with rounding specified by the operation context. The maximum value for the exponent
is `99999999`.
| xref:dw-math-functions-decimalround.adoc[decimalRound] | Returns the argument number with rounding specified by the operation context.
| xref:dw-math-functions-decimalsqrt.adoc[decimalSqrt] | Returns a number that approximates the square root of the argument with rounding specified by the operation context.
| xref:dw-math-functions-decimalsubtract.adoc[decimalSubtract] | Performs number subtraction with rounding specified by the operation context.
...
== Types
* xref:dw-math-types.adoc[Math Types]
