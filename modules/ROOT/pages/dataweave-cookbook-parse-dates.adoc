...
// include::partial$dataweave1-links.adoc[tag=dataweave1Example]

The examples use these functions:

* `replace` so that all dates match a single pattern.
* `mapObject` to run through the `date` elements. The script applies the normalizing function to each `date`.
...
You can use these regular expressions in a DW xref:dataweave-types.adoc#functions-and-lambdas[function] that first evaluates a string to see which regular expression it matches, and then converts it through the corresponding function. This function should do the following:
