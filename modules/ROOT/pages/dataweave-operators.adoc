...
In xref:dataweave.adoc[DataWeave] you can carry out many different operations on the elements of a DataWeave transform. This document serves as a reference for all of the available operators in the DataWeave language. See xref:dataweave-types.adoc#dataweave-operators-sorted-by-type[all operators sorted by type]
...
----
<2> Filters the all key:value pairs with "data" value "b" => {data:b} before the transformation block has occurred.
