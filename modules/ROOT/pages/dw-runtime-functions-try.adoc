...
The `orElseTry` and `orElse` functions also continue processing if the `try` function fails. See `orElseTry` and `orElse` for more complete examples of handling failing `try` function expressions.
