...
These example applications are configured to work with the sample flat file data and the example flat file schema provided in xref:dataweave-flat-file-schemas.adoc#full_schema_ex[Flat File Schemas].
...
image::mruntime-read-flat-file.png["A flow showing the process of reading and transforming a flat file to JSON"]
...
image::mruntime-write-flat-file.png["A flow showing the process of writing a flat file"]
...
----
. Create a file named `flatfile` in the `sr:page-aliases:
. Create a file named `flatfileschema.ffd` in the `sr:page-aliases:
...
* xref:dataweave-cookbook-set-reader-writer-props.adoc[Set Reader and Writer Properties]
* xref:4.3@mule-runtime::configuring-properties.adoc[Configure Properties]
