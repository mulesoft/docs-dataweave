...
| Shows common xref:dataweave-selectors.adoc[selector expressions] for extracting values from a data source, such as a Mule message.
//select-xml-elements
|xref:dataweave-cookbook-select-xml-elements.adoc[Select XML Elements]
| Uses a xref:ataweave-selectors.adoc#single_value_selector[single-value DataWeave selector] (`.`) to extract data from XML elements.
//defaults
...
| Shows how to use and change the settings for reading and writing xref:dataweave-formats.adoc[data formats], such as `application/csv`.
//perform-basic-transformation
| xref:dataweave-cookbook-perform-basic-transformation.adoc[Transform XML to JSON]
| Uses xref:dataweave-selectors.adoc[selectors] to perform a basic format transformation. It does not use any functions.
//map
| xref:dataweave-cookbook-map.adoc[Map Data]
| Uses xref:dw-core-functions-map.adoc[`map`] to reorganize fields in a JSON array. It uses `as` to xref:dataweave-types-coercion.adoc[coerce] a String into a Number.
//map-and-flatten
| xref:dataweave-cookbook-map-and-flatten.adoc[Map and Flatten an Array]
| Uses xref:dw-core-functions-flatMap.adoc[`flatMap`] to map objects in an array and flatten the resulting array.
//map-an-object
| xref:dataweave-cookbook-map-an-object.adoc[Map Objects]
| Uses xref:dw-core-functions-mapObject.adoc[`mapObject`] to go through the keys and values in each object, and set all the keys to upper case.
//map-an-object-key
| xref:dataweave-cookbook-map-an-object-key.adoc[Map Objects Key]
| Uses the xref:dw-core-functions-mapObject.adoc[`mapObject`] function to iterate through an array of objects and appends a new object that matches the value of the specified criteria.
...
| Uses the xref:dataweave-selectors.adoc#multi_value_selector[multi-value selector] to pick out the keys in an object that are named `"book"` and returns them as an array. It then uses xref:dw-core-functions-map.adoc[`map`] to iterate through the created array.
...
| Transforms the payload according to definitions sent in a DataWeave variable. It defines a function that applies the logic defined in the variable, using `map` and `default`.
//rename-keys
...
| Defines a function that expects to receive two inputs: a function to apply and an element to apply it on. The function is also recursively applied to the element's children. It uses xref:dw-core-functions-mapobject.adoc[`mapObject`], dw-core-functions-lower.adoc[`lower`], `is`, and xref:dataweave-flow-control.adoc#control_flow_if_else[`if/else`].
|===
...
| Uses xref:dw-core-functions-filter.adoc[`filter`] to return rows that contain a specified value in an xref:dataweave-formats-excel.adoc[XLSX] file format.
//csv-lookup
| xref:dataweave-cookbook-csv-lookup.adoc[Look Up Data in CSV File]
| Uses xref:dw-core-functions-filter.adoc[`filter`] with xref:dw-core-functions-map.adoc[`map`] to look up data in a CSV file and return a country code for each calling code found within an input array of phone numbers.
...
| Use DataWeave to read and write xref:dataweave-formats-flatfile.adoc[flat files] in a Mule application.
//read-connector
| xref:dataweave-cookbook-reader-prop-connector.adoc[Use a Reader Property through a Connector]
| Use a xref:dataweave-formats.adoc#reader_writer_properties[reader property] through a Connector operation.
//read-connector
| xref:dataweave-cookbook-writer-prop-mule.adoc[Use Dynamic Writer Properties]
| Use a xref:mule-runtime::about-mule-variables.adoc[Mule variable] as a xref:dataweave-formats.adoc#reader_writer_properties[writer] configuration value.
//pluck-function
| xref:dataweave-cookbook-pluck.adoc[Extract Key/Value Pairs with Pluck Function]
| Use the DataWeave xref:dw-core-functions-pluck.adoc[`pluck`] function to extract key-value pairs from a JSON payload and store them into xref:mule-runtime::about-mule-variables.adoc[Mule event variables].
|===
