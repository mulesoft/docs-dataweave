...
Valid values are `true` or `false`.
|`maxEntrySize`  |`Number`|`null`|Sets the maximum amount of bytes a single entry in a zip-file should become.
|`minInflateRatio`  |`Number`|`null`|Sets the ratio between de- and inflated bytes to detect zipbomb. e.g. setting the value to 1% (= 0.01d) when the compression is better than 1% for any given read package part, the parsing will fail indicating a Zip-Bomb.
