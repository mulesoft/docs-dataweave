...
:page-aliases: 3.9@mule-runtime::dataweave-language-introduction.adoc

The xref:index.adoc[DataWeave] Language is a powerful template engine that allows you to transform data to and from any kind of format (XML, CSV, JSON, Pojos, Maps, etc). In Anypoint Studio, this language can be used in:
...
* Any Mule component that accepts Mule Expression Language, through xref:xref:3.9@mule-runtime::mel-dataweave-functions.adoc[MEL DataWeave Functions]
...
[NOTE]
The `lookup` function does not support calling xref:3.9@mule-runtime::flows-and-subflows.adoc#types-of-flows[subflows].

== Calling Global MEL Functions from DataWeave Code

If you define a global xref:3.9@mule-runtime::mule-expression-language-mel.adoc[Mule Expression Language] (MEL) function in your Mule project, you can then invoke it anywhere in your DataWeave code, without need for any special syntax.
...
* xref:xref:3.9@mule-runtime::mel-dataweave-functions.adoc[MEL DataWeave Functions]
