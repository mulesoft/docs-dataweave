...
:page-aliases: 4.2@mule-runtime::dataweave-variables.adoc
...
[NOTE]
--
xref:3.9@mule-runtime::dataweave-language-introduction.adoc#_variables[DataWeave 1.0 variables] are described in the Mule 3.9 documentation.
--
...
As a best practice, declare local variables within `do` scopes (see xref:dataweave-flow-control-precedence.adoc[Flow Control in DataWeave]). The alternative method of declaring a scope with the `using` keyword is no longer recommended and available only to provide compatibility.
