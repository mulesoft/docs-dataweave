...
https://protobuf.dev/programming-guides/proto3/#oneof[Oneof^] fields, a Protobuf particularity, are represented as regular fields on DataWeave. When writing a `proto` message, a particular schema specifies what the DataWeave script needs to validate. If more than one of the fields defined is present, the script fails.
...
to achieve this, readers and writers accept https://protobuf.dev/programming-guides/proto3/#unknowns[unknown fields^] on messages. DataWeave adapts to this functionality by using certain key names.
...
https://protobuf.dev/programming-guides/proto3/#maps[Protobuf maps^] enable you to have a structure without a predefined set of keys, but with every field sharing the same value type.
