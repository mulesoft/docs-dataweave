...
DataWeave supports the use of type parameters in its function definitions, which are similar to generics in some programming languages. 
...
DataWeave also supports adding constraints for its type parameters, which in turn constraints the possible accepted inputs for a function.
