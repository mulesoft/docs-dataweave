...
The following Mule app example shows how to use the DataWeave `pluck` function to extract key/value pairs from a JSON payload and store them into flow variables with variable name as the keys and their corresponding values.

The Mule app consists of:

* A Scheduler component that triggers the flow
* A Set Payload component that sets the JSON payload
* A Transform Message component that transforms the content read from the Set Payload component by extracting the key/value pairs with the `pluck` function and put them into an array
* A For Each scope component that iterates the array payload content
* A Set Variable component that sets the flow variable name as the Key and variable value as the Value in the JSON array
...
Then, the For Each scope component iterates over the transformed values and the app logs the values of the variable results:
