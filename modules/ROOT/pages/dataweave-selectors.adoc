...
:page-aliases: 4.3@mule-runtime::dataweave-selectors.adoc

DataWeave version 2 selectors traverse the structures of objects and arrays and
...
| Index | `[<index>]` | Value of any type at selected array index. To start indexing the last item of the Array, use `-1` because `0` is reserved as the first element in the Array and `-0` does not exist.
...
You can select xref:4.3@mule-runtime::about-mule-event.adoc[Mule event] data by using
...
xref:4.3@mule-runtime::about-mule-message.adoc#_message_payload[Message Payload].

| *Attribute*
a| `attributes.<myAttributeName>`

For examples, see xref:4.3@mule-runtime::about-mule-message.adoc#_attributes[Attributes].
...
For more on Mule variables, see xref:4.3@mule-runtime::about-mule-variables.adoc[Variables in Mule Apps].
...
For more on flows, see xref:4.3@mule-runtime::about-flows.adoc[Flows and Subflows].
|===
