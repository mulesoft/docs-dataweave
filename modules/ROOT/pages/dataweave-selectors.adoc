...
such as the payload, an attribute or variable data within the Mule event, you
use predefined Mule Runtime variables, such as `payload`, `attributes`, and `vars`.

Extracted values are handled as a literal values of one of the supported DataWeave value types.
