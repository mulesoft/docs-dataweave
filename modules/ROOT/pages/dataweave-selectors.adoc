...
| Index | `[<index>]` | Value of any type at selected array index. `0` is reserved as the first element in the Array and `-0` does not exist, so DataWeave starts indexing the last item of the Array from `-1`.
