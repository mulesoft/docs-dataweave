...
:page-aliases: 3.8@mule-runtime::dataweave-reference-documentation.adoc
...
In Anypoint Studio, there are two ways to achieve this. You can either manually add the attributes to the project's XML or use the graphical interface by selecting the element from the tree view in the input section and clicking the gear icon.
...
xref:3.8@mule-runtime::mule-expression-language-mel.adoc[Mule Expression Language (MEL)] expression.
...
You can take any *Outbound Properties* in the Mule message that arrives to the DataWeave transformer and use it in your transform body. To refer to it, simply call it through the matching xref:3.8@mule-runtime::mule-expression-language-mel.adoc[Mule Expression Language (MEL)] expression.
...
Your DataWeave code can call any function that you define as a global xref:3.8@mule-runtime::mule-expression-language-mel.adoc[Mule Expression Language (MEL)] function, as long as it is correctly defined in the Mule project where your Transform Message element sits.
