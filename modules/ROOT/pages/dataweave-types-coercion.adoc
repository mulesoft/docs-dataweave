...
[NOTE]
To maintain consistency across all formats, starting with DataWeave 2.5, writing number values directly doesn’t preserve formatting and strips trailing zeroes. Format metadata is only used to parse the provided string into the correct number representation. To retain this behavior, you can re-enable it by using the `stripTrailingZeroes` property. See xref:dataweave-system-properties.adoc[supported data formats]
