...
The following example converts a number to a string with decimal representation.
...
Note: In order to be consistent across all formats, since DataWeave 2.5 writing number values directly will not preserve format and will strip trailing zeroes. Format metadata will only be used to parse the provided string into the right number representation. If you still need this behavior you can re-enable it by using the `stripTrailingZeroes` property found here:
xref:dataweave-system-properties.adoc[Supported Data Formats]
