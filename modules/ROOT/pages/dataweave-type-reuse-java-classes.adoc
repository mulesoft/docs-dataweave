...
If a Java class is not present in the table, DataWeave treats the class as a `JavaBean` and maps it to a DataWeave `Object` type. The keys in that object match the names of the properties in the Java class and their values correspond to the DataWeave type that marches the property class. The following example shows how DataWeave takes all properties from the Java getters:
