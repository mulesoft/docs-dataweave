...
The following Mule flow transforms comma-separated CSV data into pipe-separated (`|`) CSV data. Instead of setting a literal value in the script, the example selects the pipe value from a xref:4.3@mule-runtime::about-mule-variables.adoc[Mule variable], which is part of a Mule event that travels through data-processing components in the flow.
...
For further guidance with sample data, see xref:4.3@mule-runtime::transform-preview-transformation-output-studio-task.adoc[Previewing the Output of a Transformation].

== See Also

* xref:dataweave-formats.adoc[DataWeave Formats]
* xref:4.3@mule-runtime::about-components.adoc[Core Components]
