...
The following Mule flow transforms comma-separated CSV data into pipe-separated (`|`) CSV data. Instead of setting a literal value in the script, the example selects the pipe value from a xref:mule-runtime::about-mule-variables.adoc[Mule variable], which is part of a Mule event that travels through data-processing components in the flow.
