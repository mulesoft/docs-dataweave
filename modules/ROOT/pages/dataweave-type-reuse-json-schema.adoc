...
* Formats on JSON schema string types aren't supported and are transformed to `String` without any validation or restriction.
* Restrictions on number types such as maximums or minimums aren't supported and are be transformed to `Number` without any validation or restriction.
* Patterns aren't supported.

=== Object types

Non-required properties in JSON Schema object types are marked as optional properties in the DataWeave Object type. 
