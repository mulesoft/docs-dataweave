...
Those buffer files remain in the temporary directory until the streams referencing them are closed. Note that the closure normally happens when flows complete their execution, so long-running and particularly concurrent executions could involve having many of these files living at once.
...
This system property determines the size in bytes of the in-memory input and output buffers used in DataWeave to keep the processed inputs and outputs. If a payload exceeds this size, it is stored in `dw-buffer-index-${count}.tmp` and `dw-buffer-output-${count}.tmp` temporary files. The default buffer size is 8192 bytes (8 KB).
