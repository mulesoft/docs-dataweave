...
Returns the `DataFormatDescriptor` based on the specified `dw::module::Mime::MimeType` specified or `null` if
...
This example search for a JSON `DataFormatDescriptor` and an unknown `DataFormatDescriptor`.
