...
Inside the `plugin` configuration, add a `tests` element configured to use parameters that meet your specific testing needs:
...
|`boolean`
|`true`
|When set to `true`, the test suite generates an HTML report about the execution.

|`coverageEnabled`
|`boolean`
|`false`
|When set to `true`, the test suite generates a coverage report about the execution.
...
Inside the `plugin` configuration, add a `docs` element configured with parameters that meet your desired documentation needs:
