...
* `sonar` +
Generates a valid https://www.sonarsource.com/products/sonarqube/[Sonar^] coverage report for your test suite.
* `html` +
...
The Maven plugin for DataWeave can run cryptographic taint analysis at compile time to help identify potential security vulnerabilities in your DataWeave code. The analysis tracks literal algorithm values that flow into parameters annotated with xref:dw-crypto-annotations.adoc[`@CryptographicSink`] and causes the build to fail when an algorithm matches your configured insecure list. For more details about the annotation, see xref:dw-crypto-annotations.adoc[Crypto Annotations].
...
* xref:dataweave-extension-plugin.adoc[Installing and Using the DataWeave Extension]
