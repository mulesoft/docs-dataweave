...
| Annotation that allows internal directives (var, fun, type, ns) to be visible
to specific components beyond their own component. This annotation provides
fine-grained control over internal visibility by explicitly listing which
components can access the internal declaration.

Example:
```
@VisibleTo(components = ["component-b", "component-c"])
internal var sharedSecret = 42
```

_Introduced in DataWeave 2.12.0. Supported by Mule 4.12.0 and later._
