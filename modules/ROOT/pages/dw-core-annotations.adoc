...
| Annotation that marks an `internal` directive (`fun`, `var`, `type`, `ns`)
as visible to the listed components in addition to its own component. Use it
to share helpers between libraries that you own without making them public.
Component names match the `name` field in the `META-INF/dw-components.dwl`
descriptor.

`@VisibleTo` is only valid on `internal` directives. Applying it to a
`private` directive, or to a directive without any visibility keyword,
