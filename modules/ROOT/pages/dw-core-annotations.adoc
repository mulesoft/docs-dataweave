...
| Annotation that marks an `internal` directive (`fun`, `var`, `type`, `ns`) as visible to listed components in addition to its own component. Use it to share helpers across libraries you own without making them public. Component names must match the `name` field in the `META-INF/dw-components.dwl` descriptor.
