...
The following example declares the `NameType` from the `User` type. Using the dot selector in `User.name` to declare the `name` variable a `NameType` enables the `typeOf()`  function to return the primitive type, `"String"`. Notice how the DataWeave variable `name` is declared as a `NameType` value "Seba":
...
The following example shows how type selection can also be used with `Union` types that consist of `Object` types:
...
== Select Types to Preserve Metadata
